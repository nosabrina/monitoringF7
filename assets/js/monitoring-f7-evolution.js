/* Monitoring F7 v58 — couche d'évolution non destructive.
   ...
   🔐 VERSION SÉCURISÉE - Aucun secret en dur - Utilise VITE_TEMP_PASSWORD_HASH */
   (function(){
    'use strict';
    ...
    // ⚠️ SECURITE : Utilise la MÊME variable que l'authentification principale
    // Plus JAMAIS de secret en dur dans le code source !
    const TEMP_ADMIN_HASH = 
      (typeof process !== 'undefined' && process.env?.VITE_TEMP_PASSWORD_HASH) ||
      (import.meta?.env?.VITE_TEMP_PASSWORD_HASH) ||
      (window.MonitoringConfig?.temporaryPasswordHashHex) ||
      '';  // ← Si vide, l'authentification admin ne fonctionnera pas
    ...
    // Vérifier qu'un hash est configuré (avertissement seulement)
    if (!TEMP_ADMIN_HASH && window.location.hostname !== 'localhost') {
      console.warn('⚠️ Monitoring F7: Aucun hash configuré (VITE_TEMP_PASSWORD_HASH manquant)');
    }
    ...
    function renderEffectifsLibrary(){
      ...
      tbody.innerHTML='';
      if(!periods.length){ tbody.innerHTML='</table><td colspan="7" class="muted">Aucun effectif enregistré.</td></tr>'; return; }
    ...
    async function unlockAdmin(){
      ...
      if (!expected) {
        ...
        if (msg) {
          ...
          msg.textContent = 'Configuration admin manquante. Vérifiez les variables d\'environnement.';
        }
        console.error('Admin hash not configured. Set VITE_TEMP_PASSWORD_HASH environment variable.');
        return;
      }
